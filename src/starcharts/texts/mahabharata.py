"""Krishna's birth (Janmashtami) and the Mahabharata war's planetary-
omen verses, encoded as search constraints.
...
verses below have been. The war "omen" verses (Udyoga/Bhishma Parva) are
the opposite: richer in planetary detail, and both
...
Udyoga Parva) -- see the per-verse notes below.
...
MAHABHARATA WAR OMENS -- citation-checked against the Sanskrit critical
edition (GRETIL's Bhismaparvan text, mbh_06_u.htm), not just an English
translation.

**Bhishma Parva 06,003 (Vyasa's narration to Dhritarashtra), verified
shloka by shloka**:
- 06,003.013c-d: "maghāsu aṅgārakaḥ vakraḥ, śravaṇe ca bṛhaspatiḥ" --
  Mars retrograde in Magha, and Jupiter in Shravana, as one clause: the
  retrograde qualifier applies to the Mars-in-Magha placement, not a
  separate, optional detail.
- Shani afflicting Rohini (separately attested).
- 06,003.015: "śyāmo grahaḥ ... ākramya jyeṣṭhām tiṣṭhati" -- "the dark
  ...
  texts, so this is encoded generically as "a node" (checks Rahu or
  Ketu, whichever fits) rather than committing to one.
- 06,003.011c: "śveto grahas ... citrām samatikramya tiṣṭhati" -- "a
  ...
  relationship, this is almost certainly a different moment within the
  same extended omen narration, not the same instant as the Jyeshtha
  placement -- consistent with the omens describing a run-up period of
  months, not one frozen instant. Not required simultaneously with the
  Jyeshtha node placement; not encoded as a joint constraint.
...
**Udyoga Parva 5.141 (Karna's speech to Krishna), also Sanskrit-
verified, via a Devanagari reader edition (rikamrit.com)**:
...
- 5.141.8: "kṛtvā ca aṅgārakaḥ vakraṃ jyeṣṭhāyāṃ madhusūdana, anurādhāṃ
  ...
  event, not a choice between two nakshatras: retrograde motion runs
  backward through the zodiac, and Anuradha (nakshatra 16) precedes
  Jyeshtha (17) in normal order, so "retrograde in Jyeshtha, heading
  toward Anuradha" is internally consistent astronomy.
- 5.141.9: "citrāṃ pīḍayate grahaḥ" -- "a planet afflicts Chitra." The
  planet is genuinely unnamed in this verse itself; not encoded.
- 5.141.10: "rāhur arkam upaiṣyati" -- "Rahu approaches the sun" --
  real in the source, not encoded (no orb given).

Both `war_omen_profile_bhishma_parva` and `war_omen_profile_udyoga_parva`
are Sanskrit-verified to the same standard.
...
def war_omen_profile_bhishma_parva_mars_circuitous_variant(
    ...
    """Sanskrit-verified against GRETIL's Bhismaparvan text, 06,003.017a:
    "vakrānuvakraṃ kṛtvā ca śravaṇe pāvakaprabhaḥ" -- "having
...
def war_omen_profile_udyoga_parva(
    ...
    """Sanskrit-verified against Mahabharata 5.141.7-8. Shani afflicting
    the "Prajapatya" nakshatra (Rohini's Vedic deity -- same claim as
    Bhishma Parva, independently confirmed), and Mangala retrograde in
    Jyeshtha (5.141.8: "kṛtvā ca aṅgārakaḥ vakraṃ jyeṣṭhāyāṃ"; verse 8
    also has Mars then approaching Anuradha as a consequence of that
    retrograde motion, not encoded as a separate simultaneous placement
    since it's the same single continuous event). Does not include the
    unnamed-planet-at-Chitra (5.141.9) or Rahu-near-Sun (5.141.10)
    details -- both real in the source but not specific enough to
